...
public:
  ...
  unsigned char GetCurrentState() { return current_state_; }
...
protected:
  ...
  void ExternalEvent(unsigned char newState, const std::shared_ptr<EventData> pData = nullptr);
...
private:
  ...
  void StateEngine(void, const std::shared_ptr<EventData> pData = nullptr);
  void StateEngine(const StateMapRow* const pStateMap, const std::shared_ptr<EventData> pData = nullptr);
  void StateEngine(const StateMapRowEx* const pStateMapEx, const std::shared_ptr<EventData> pData = nullptr);
};
...
#define GUARD_DECLARE(stateMachine, guardName, eventData) \
	bool GD_##guardName(const std::shared_ptr<eventData>); \
...
#define GUARD_DEFINE(stateMachine, guardName, eventData) \
	bool stateMachine::GD_##guardName(const std::shared_ptr<eventData> data)

#define ENTRY_DECLARE(stateMachine, entryName, eventData) \
	void EN_##entryName(const std::shared_ptr<eventData>); \
...
#define ENTRY_DEFINE(stateMachine, entryName, eventData) \
	void stateMachine::EN_##entryName(const std::shared_ptr<eventData> data)
